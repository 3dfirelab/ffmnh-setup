...
###################################################
    ...
    parser.add_argument('-i','--input', help='Input MNH dir',required=True)
    parser.add_argument('-o','--output', help='Output MNH dir',required=True)
    ...
    #create dir
    outputForeFire= f'{args.output}/ForeFire/'
    tools.ensure_dir(outputForeFire)
    tools.ensure_dir(f'{outputForeFire}/Outputs/')

    dir_input = args.input
    if dir_input is None: dir_input = '../006_mnh/'
    domainNumber = int(args.domainNumber) if (args.domainNumber is not None) else 1
    ...
    MNH_domain['nz'] = 1 
    MNH_domain['fx'] = MNH_domain['nx']
    MNH_domain['fy'] = MNH_domain['ny']
    MNH_domain['fz'] = MNH_domain['nz']
    MNH_domain['ft'] = 1 
    MNH_domain['nt'] = 1 

    dummyLayer = xr.DataArray(
                                np.full((MNH_domain['nt'], MNH_domain['nz'], MNH_domain['ny'], MNH_domain['nx'] ), 1),
                                dims=("nt", "nz", "ny", "nx")
                             )
    fuelLayer = xr.DataArray(
                                np.full((MNH_domain['ft'], MNH_domain['fz'], MNH_domain['fy'], MNH_domain['fx']), 1),
                                dims=("ft", "fz", "fy", "fx")
                             )
    '''
    ...
    fuelmap = fuelLayer.copy()
    fuelmap.attrs["type"] = "fuel"
    fuelmap = fuelmap.astype('int16')
    fuelmap = fuelmap
    ...
    heatFlux = dummyLayer.copy().astype('int16')
    heatFlux[:,:]=0
    heatFlux.attrs["type"] = "flux"
    heatFlux.attrs["indices"] = 0
    heatFlux.attrs["model0name"] = "heatFluxBasic"

    #moisture = fuelmap.copy()
    #moisture[:,:]=0.1
    #moisture.attrs["type"] = "data"

    vaporFlux = dummyLayer.copy().astype('int16')
    vaporFlux[:,:]=0
    vaporFlux.attrs["type"] = "flux"
    vaporFlux.attrs["indices"] = 0
    vaporFlux.attrs["model0name"] = "vaporFluxBasic"
    ...
    #MERDE
    #ds['moisture'] = dummyLayer.copy().astype(float) * 20
    #ds['moisture'].attrs["type"] = "data"
    ...
    '''
    ds['altitude'] = dummyLayer.copy().astype(float) * 0
    ds['altitude'].attrs["type"] = "data"
    ...
    #and save
    print(f'save {outputForeFire}/mydata.nc')
    if os.path.isfile(f'{outputForeFire}/mydata.nc'):
        os.remove(f'{outputForeFire}/mydata.nc')
    ds.to_netcdf(f'{outputForeFire}/mydata.nc')
